...
inline double calculateP(int j, int Nx) { // фазовая скорость P
    return (j < Nx / 2.0) ? pow(0.1, 2) : pow(0.2, 2);
}

inline double calculateTao(int Nx, int Ny) { // величина шага Tao по времени
    return (Nx <= 1000 && Ny <= 1000) ? 0.01 : 0.001;
}

inline void initArrays(int Nx, int Ny, double* U0, double* U1, double* U2, double* P) { // инициализация массивов: изначально U0 - момент времени t(-1), U1 - t(0), U2 - t(1)
    for (int i = 0; i != Nx; i++) {
        for (int j = 0; j != Ny; j++) {
            *(U0 + i * Ny + j) = 0;
            *(U1 + i * Ny + j) = 0;
...
double functionF(int i, int j, int n, int Nx, int Ny, int Sx, int Sy) { // значение функции источника в момент времени n на узле (i, j) 
    double f0 = 1.0;
    double t0 = 1.5;
    double gamma = 4.0;
...
void algorithmStep(int i, int j, int n, int Nx, int Ny, int Sx, int Sy, double Hx, double Hy, double* U0, double* U1, double* U2, double* P) { // шаг алгоритма (двухслойная явная схема)
    double tao = calculateTao(Nx, Ny);
...
void processAlgorithm(int Nx, int Ny, int Nt, int Sx, int Sy, double Hx, double Hy, double* U0, double* U1, double* U2, double* P) { // все шаги алгоритма по времени и по пространству
    for (int n = 1; n != Nt; n++) {
        double Umax = 0.0;
...
void saveArrayToFile(int Nx, int Ny, const double* dataArray, const char* filename) { // сохранение результатов в файл для рисования распределения
...
int main(int argc, char* argv[]) {
    int Nx = 600, Ny = 600; // mash size
    int Nt = 2500; // step count
